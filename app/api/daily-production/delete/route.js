...
export async function POST(req) {
  try {
    ...
    if (workAs === 'helper') {
      // Ensure these fields are explicitly not passed if they are required/validated for 'operator'
      // You might need to adjust based on your actual Mongoose schema for `DailyProduction`
      // For simplicity, we assume removing them from the payload is enough.
      delete payload.machineType;
      delete payload.uniqueMachine;
...
export async function GET(req) {
  try {
    ...
    const data = await DailyProduction.find(query).sort({ date: -1 });
...
// ===================================
// DELETE: Delete a production entry by ID
// ===================================
export async function DELETE(req) {
  try {
    await connectDB();
    // Expecting the unique MongoDB _id from the request body
    const { _id } = await req.json();

    if (!_id) {
      return NextResponse.json(
        { error: 'Document ID (_id) is required for deletion.' },
        { status: 400 }
      );
    }

    const result = await DailyProduction.deleteOne({ _id });

    if (result.deletedCount === 0) {
      // If deletedCount is 0, the document wasn't found
      return NextResponse.json(
        { error: 'Production entry not found or already deleted.' },
        { status: 404 }
      );
    }

    // Success response
    return NextResponse.json({ message: 'Production entry successfully deleted.' }, { status: 200 });

  } catch (error) {
    console.error('Error deleting production entry:', error);
    return NextResponse.json(
      { error: 'Failed to delete production entry. Please try again.' },
      { status: 500 }
    );
  }
}
